...
export default function App() {
  ...
  const {
    activeListings, myListings, myOffers, receivedOffers, myTxHistory,
    offers, actionLoading,
    createAndLockListing, cancelListing, buyListing, makeOffer, acceptOffer, rejectOffer,
  } = useListings(wallet);

  const { toasts, add: addToast } = useToast();

  useEffect(() => {
    const saved = localStorage.getItem('xchange_dark');
  ...
  const handleCreateListing = async (data) => {
    try {
      const r = await createAndLockListing(data);
      addToast(`✅ Asset locked! Listing is now live. TX: ${r.txid?.slice(0, 16)}…`);
      setTab('market');
    } catch (e) {
  ...
  const handleBuy = async (listing) => {
    try {
      ...
      addToast(`✅ Purchase complete! Fees: ${r.fees?.total} sats. TX: ${r.txid?.slice(0, 16)}…`);
      setSelected(null);
      setTab('market');
      refreshBalances();
    } catch (e) {
      addToast(`❌ Buy failed: ${e.message}`, 'error');
    }
  };

  const handleOffer = async (listingId, listingName, btcAmount) => {
    try {
      const r = await makeOffer(listingId, listingName, btcAmount);
      addToast(`✅ BTC locked in escrow! Offer sent. TX: ${r.txid?.slice(0, 16)}…`);
    } catch (e) {
      addToast(`❌ Offer failed: ${e.message}`, 'error');
    }
  };
  ...
  const handleCancel = async (id) => {
    try {
      ...
      addToast(`↩️ Listing cancelled. Asset unlocked. TX: ${r.txid?.slice(0, 16)}…`);
    } catch (e) {
  ...
  const handleAcceptOffer = async (offerId) => {
    try {
      ...
      addToast(`✅ Offer accepted! BTC sent to you (minus fees). TX: ${r.txid?.slice(0, 16)}…`);
    } catch (e) {
  ...
  const handleRejectOffer = (offerId) => {
    rejectOffer(offerId);
    addToast('Offer rejected. Buyer BTC will be refunded from escrow.');
  };

  const goTo = (t) => { setTab(t); setSelected(null); };
    ...
    <>
      ...
      <div className="app">
        <header className="header">
          <div className="header-left" onClick={() => goTo('market')} style={{ cursor: 'pointer' }}>
            <Logo size={36} />
            <div>
          ...
          <div className="header-right">
            <button className="btn btn-ghost btn-sm theme-btn" onClick={() => setDark(d => !d)}>{dark ? '☀️' : '🌙'}</button>
        ...
        <nav className="nav">
          {NAV.map(n => (
            <button key={n.id} className={`nav-item${tab === n.id && !selectedListing ? ' active' : ''}`} onClick={() => goTo(n.id)}>
              {n.label}
              {n.badge && <span style={{ marginLeft: 6, background: 'var(--orange)', color: '#fff', fontSize: 9, fontWeight: 800, padding: '1px 6px', borderRadius: 99, fontFamily: 'var(--mono)', letterSpacing: '0.5px', verticalAlign: 'middle' }}>{n.badge}</span>}
            </button>
          ))}
        </nav>

        <main className="main">
          {tab === 'market' && !selectedListing && (
            <MarketPage listings={activeListings} offers={offers} onSelect={setSelected} onCreateListing={() => goTo('create')} wallet={wallet} />
          )}
          {tab === 'market' && selectedListing && (
            <ListingDetailPage
          ...
          {tab === 'create' && (
            <CreateListingPage wallet={wallet} onSubmit={handleCreateListing} onCancel={() => goTo('market')} actionLoading={actionLoading} />
          )}
          {tab === 'dashboard' && (
            <DashboardPage
              wallet={wallet}
...
function AppStyles() {
  return (
    <style>{`
      ...
      .theme-btn { font-size:16px !important; padding:7px 10px !important; }
      ...
      .btn { display:inline-flex; align-items:center; gap:6px; padding:9px 18px; border-radius:10px; border:none; cursor:pointer; font-family:var(--font); font-weight:700; font-size:13px; transition:all 0.16s; }
      .btn:disabled { opacity:0.45; cursor:not-allowed; }
      .btn-orange { background:var(--orange); color:#fff; }
      .btn-orange:not(:disabled):hover { background:#ff7820; transform:translateY(-1px); box-shadow:0 4px 16px rgba(255,98,0,0.35); }
      ...
      .inp { width:100%; padding:10px 14px; border-radius:10px; background:var(--bg3); border:1.5px solid var(--border); color:var(--text); font-family:var(--font); font-size:13px; outline:none; transition:border-color 0.15s; }
      ...
      .hero::after { content:''; position:absolute; inset:0; background:radial-gradient(ellipse at 50% 120%,rgba(255,98,0,0.1),transparent 65%); pointer-events:none; }
      ...
      .listing-card:hover { border-color:var(--orange); transform:translateY(-2px); box-shadow:0 8px 32px rgba(255,98,0,0.15); }
      ...
      .escrow-note { font-size:11px; color:var(--muted); line-height:1.6; margin-top:12px; padding:10px; background:var(--bg3); border-radius:8px; }
      ...
      .spinner-sm { width:12px; height:12px; border:2px solid rgba(255,255,255,0.3); border-top-color:#fff; border-radius:50%; animation:spin 0.7s linear infinite; flex-shrink:0; }

      .toast { background:var(--bg2); border:1.5px solid var(--border); padding:12px 16px; border-radius:10px; font-size:13px; box-shadow:var(--card-shadow); min-width:220px; max-width:340px; }
      .toast-success { border-color:var(--green); }
      .toast-error { border-color:var(--red); }
      .toast-info { border-color:var(--orange); }

      .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.65); z-index:400; display:flex; align-items:center; justify-content:center; padding:20px; }
      .modal { background:var(--bg2); border:1.5px solid var(--border); border-radius:var(--radius); padding:28px; max-width:460px; width:100%; box-shadow:0 24px 64px rgba(0,0,0,0.5); }
      ...
      .dash-addr { font-family:var(--mono); font-size:12px; color:var(--muted); margin-bottom:20px; }
      ...
      .dash-tab { padding:7px 14px; border-radius:8px; cursor:pointer; font-size:12px; font-weight:700; color:var(--muted); border:none; background:none; font-family:var(--font); transition:all 0.14s; }
      .dash-tab.active { background:var(--orange); color:#fff; }

      .table-wrap { overflow-x:auto; border:1px solid var(--border); border-radius:var(--radius); }
      .table { width:100%; border-collapse:collapse; }
      .table th { font-size:11px; text-transform:uppercase; letter-spacing:0.5px; color:var(--muted); padding:11px 16px; text-align:left; border-bottom:1px solid var(--border); background:var(--bg3); }
      .table td { padding:12px 16px; font-size:13px; border-bottom:1px solid var(--border); }
      .table tr:last-child td { border-bottom:none; }
      .table tr:hover td { background:var(--bg3); }

      .status-badge { padding:3px 10px; border-radius:99px; font-size:11px; font-weight:800; font-family:var(--mono); }
      .status-active { background:var(--green-dim); color:var(--green); }
      .status-sold { background:var(--orange-dim); color:var(--orange); }
      ...
      .status-refunded { background:var(--muted); color:var(--bg); }
      .status-locked { background:var(--green-dim); color:var(--green); }
      .chip { display:inline-flex; align-items:center; padding:3px 10px; border-radius:99px; font-size:11px; font-weight:800; font-family:var(--mono); }
      .chip-buy { background:var(--red-dim); color:var(--red); }
      .chip-sell { background:var(--green-dim); color:var(--green); }
