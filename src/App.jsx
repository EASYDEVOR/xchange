...
export default function App() {
  ...
  const {
    ...
    makeOffer, acceptOffer, rejectOffer, markTransferComplete,
  } = useListings(wallet);
  ...
  const handleCreateListing = async (data) => {
    try {
      ...
      addToast('✅ Listing created! $3 fee sent to treasury.');
      setTab('market');
  ...
  const handleBuy = async (listing) => {
    try {
      ...
      addToast('✅ $3 fee paid. Private chat opened with seller!');
      refreshBalances();
      return r;
    } catch (e) { addToast(`❌ ${e.message}`, 'error'); }
  };
  ...
  const handleOffer = async (listingId, listingName, sellerAddress, btcAmount) => {
    try {
      ...
      addToast('✅ $3 fee paid. Offer sent — chat opens when accepted.');
      return r;
    } catch (e) { addToast(`❌ ${e.message}`, 'error'); }
  };
  ...
  const handleAcceptOffer = async (offerId) => {
    try {
      ...
      addToast('✅ Offer accepted! Private chat opened with buyer.');
      return r;
    } catch (e) { addToast(`❌ ${e.message}`, 'error'); }
  };
  ...
  const handleCancel = (id) => {
    cancelListing(id);
    addToast('Listing cancelled.');
  };

  const handleRejectOffer = (offerId) => {
    rejectOffer(offerId);
    addToast('Offer rejected.');
  };

  const handleMarkComplete = (listingId) => {
    markTransferComplete(listingId);
    addToast('✅ Listing marked as complete!');
    setSelected(null);
    setTab('market');
  };
...
function AppStyles() {
  return (
    <style>{`
      ...
      .modal{background:var(--bg2);border:1.5px solid var(--border);border-radius:var(--radius);padding:28px;max-width:460px;width:100%;box-shadow:0 24px 64px rgba(0,0,0,.5)}
      ...
      .table th{font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--muted);padding:11px 16px;text-align:left;border-bottom:1px solid var(--border);background:var(--bg3)}
      ...
      .status-pending_transfer{background:var(--orange-dim);color:var(--orange)}
      ...
      @media(max-width:600px){.main{padding:16px}.hero{padding:28px 16px}.listings-grid{grid-template-columns:1fr}.filters-bar{gap:8px}.detail-layout{grid-template-columns:1fr}}
    `}</style>
  );
}
